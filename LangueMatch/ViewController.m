...
#define kFirebaseAddress @"https://simplechat2.firebaseio.com/forums/"
...
#pragma mark - View Setup/Lifecycle

-(instancetype) initWithGroupId:(NSString *)groupId archiveMessages:(BOOL)archive
{
    if (self = [super init]) {
        _groupId = groupId;
        _archiveMessages = archive;
    }
    return self;
}
...
{
    return [self initWithGroupId:groupId archiveMessages:NO];
}

-(instancetype) init
{
    return [self initWithGroupId:@"Group" archiveMessages:NO];
}
...
{
    self.messageFirebase = [[Firebase alloc] initWithUrl:[NSString stringWithFormat:@"%@%@/messages", kFirebaseAddress, _groupId]];
    self.typingFirebase = [[Firebase alloc] initWithUrl:[NSString stringWithFormat:@"%@%@/typing", kFirebaseAddress, _groupId]];
    self.memberFirebase = [[Firebase alloc] initWithUrl:[NSString stringWithFormat:@"%@%@/members", kFirebaseAddress, _groupId]];
